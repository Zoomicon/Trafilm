...
(function ($) {
    ...
    linkify.plugins = {
        linkifyStandard: function (html) {
            return html.replace(/(^|[\[\{”"'(\s]|&lt;)(www\..+?\..+?)((?:[:?]|\.+)?(?=(?:\s|$)|&gt;|[)\]\}”"',]))/g, '$1<a href="<``>://$2">$2</a>$3').replace(/(^|[\[\{“"'(\s]|&lt;)((?:(?:https?|ftp):\/\/|mailto:).+?)((?:[:?]|\.+)?(?=(?:\s|$)|&gt;|[)\]\}”"',]))/g, '$1<a href="$2">$2</a>$3').replace(/"<``>/g, '"http')
        },
        linkifyNewWindow: function (html) {
            return html.replace(/(^|[\[\{”"'(\s]|&lt;)(www\..+?\..+?)((?:[:?]|\.+)?(?=(?:\s|$)|&gt;|[)\]\}”"',]))/g, '$1<a href="<``>://$2" target="_blank">$2</a>$3').replace(/(^|[\[\{“"'(\s]|&lt;)((?:(?:https?|ftp):\/\/|mailto:).+?)((?:[:?]|\.+)?(?=(?:\s|$)|&gt;|[)\]\}”"',]))/g, '$1<a href="$2" target="_blank">$2</a>$3').replace(/"<``>/g, '"http')
        }
    };
